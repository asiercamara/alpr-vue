...
<AccordionGroup>
  ...
  <Accordion title="Continuous mode (default: on)">
    ...
    When continuous mode is **off**, the camera stops automatically as soon as the first plate is confirmed. This is useful for single-plate capture workflows — for example, manually logging vehicles one at a time at a checkpoint — where you want to review each plate before moving on.

    Toggle continuous mode back on at any time to resume uninterrupted scanning.
  </Accordion>

  <Accordion title="Skip duplicates (default: on)">
    When skip duplicates is **on**, any plate whose text already appears in your current detection history is silently ignored. This keeps your history clean when the same vehicle stays in view for an extended period.

    Turn this **off** if you need to record every detection event regardless of whether that plate has been seen before — for example, when logging entry and exit times for the same vehicle.
  </Accordion>
