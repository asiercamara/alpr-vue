...
```bash
pnpm docs:dev      # VitePress dev server (docs only, full HMR)
pnpm docs:build    # builds docs into dist/docs/
...
In `pnpm dev` (main app), `/docs/*` is served from the pre-built `dist/docs/` via a Vite middleware plugin (`serveDocsPlugin` in `vite.config.ts`). Requires a prior `pnpm docs:build`. For active docs editing use `pnpm docs:dev`.
